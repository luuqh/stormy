     ...
     &                            ) )
      ...
      if (.not.WEST_INTER) then
        do j=JstrV,Jend
          cx=dtfast*sqrt(0.5D0*g*(h(Istr-1,j-1)+h(Istr-1,j)))
     &                    *0.5D0*(pm(Istr-1,j-1)+pm(Istr-1,j))
     ...
     &                        *vmask(Istr-1,j)
      ...
      if (.not.EAST_INTER) then
        do j=JstrV,Jend
          cx=dtfast*sqrt(0.5D0*g*(h(Iend+1,j-1)+h(Iend+1,j)))
     &                    *0.5D0*(pm(Iend+1,j-1)+pm(Iend+1,j))
