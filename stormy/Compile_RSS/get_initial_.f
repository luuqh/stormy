     ...
     &       /ocean_vbar/vbar
      real time_scale
      integer*4 itrc
      integer*4 ncid, indx, varid,  ierr, lstr, lvar, latt, lenstr,
     ...
     &                            'read time and record indices.'
        ...
        nrechis=0
        nrecavg=0
        if (mynode.eq.0) write(stdout,'(6x,2A,G12.4,1x,A,I4)')
     &          'GET_INITIAL -- ',
     &          'Processing data for time =', tdays, 'record =', indx
      ...
      if (ierr .eq. nf_noerr) then
        ierr=nf_fread (zeta(-1,-1,1), ncid, varid,
     &                                         indx, r2dvar)
        if (ierr .ne. nf_noerr) then
      ...
      if (ierr .eq. nf_noerr) then
        ierr=nf_fread (ubar(-1,-1,1), ncid, varid,
     &                                         indx, u2dvar)
        if (ierr .ne. nf_noerr) then
     ...
     &                                      ininame(1:lstr)
      ...
      if (ierr .eq. nf_noerr) then
        ierr=nf_fread (vbar(-1,-1,1), ncid, varid,
     &                                         indx, v2dvar)
        if (ierr .ne. nf_noerr) then
