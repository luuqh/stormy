  ...
  1   i=3-itslp
       cff=time+0.5D0*dt
        ...
        if (ierr.ne.0) goto 7
        ierr=nf_get_var1_double(ncidfrc, slp_tid, slp_rec, cff)
        if (ierr .ne. nf_noerr) then
          write(stdout,6) 'slp_time', slp_rec
          goto 99
        endif
        sms_time(i)=cff*day2sec+slp_cycle*slp_ncycle
        if (sms_time(itslp).eq.-1.D+20) sms_time(itslp)=sms_time(i)
        if (lslpgrd.eq.1) then
          ierr=nf_fread (slpg(-1,-1,i), ncidfrc, slp_id,
     &                                             slp_rec, r2dvar)
        else
          ierr=nf_get_var1_double (ncidfrc,slp_id,slp_rec,slpp(i))
        endif
        if (ierr .ne. nf_noerr) then
       ...
       itslp=i
       if (mynode.eq.0) write(stdout,'(6x,A,1x,A,1x,g12.4,1x,I4)')
     &           'GET_SLP --',
