...
elseif strcmp(OGCM,'ECCO')
  ...
  %  url = 'http://ecco.jpl.nasa.gov/datasets/kf080/kf080_'; 
...
if makeclim==1 | makebry==1
  ...
  for Y=Ymin:Ymax
	...
	nc_clm=[];
      ...
      %
      % Check if there are OGCM files for the previous Months
      %
      for aa= 1:itolap_a
      fname=[OGCM_dir,OGCM_prefix,'Y',num2str(Ym),'M',num2str(aa),'.cdf'];
      if exist(fname)==0
...
%        Mm=M-aa;Ym=Y;
	...
	interp_OGCM(OGCM_dir,OGCM_prefix,Y,M,Roa,interp_method,...
      ...
      %
      % Read the OGCM files for the next months
      %
      fname=[OGCM_dir,OGCM_prefix,'Y',num2str(Yp),'M',num2str(Mp),'.cdf'];
%	disp([fname])
      if exist(fname)==0
	disp(['   No data for the next month'])
%	Mp=M;
%	Yp=Y;
%	for aa=1:itolap_p    
%	  tndx_OGCM(aa)=ntimes;
